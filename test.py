        ...
        try:
            if float(self.Weight_ip.text()) <= 5:
                ...
                if gv.SettingsInputs.value('Screensaver') == 'Images':
                    ...
                    ui.imgAdW.mediaPlayer.setMedia(QMediaContent(QUrl.fromLocalFile("Saved videos\Img_SS.avi")))
...
class BMI(QMainWindow):
    ...
    def get_bmi(self):
        self.BMindex = str("{:.2f}".format(float(self.Ht)/((float(self.Wt)/100)**2)))
